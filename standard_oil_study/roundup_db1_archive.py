...
class Case(Base):
    """The individual Supreme Court cases"""
    __tablename__ = 'cases'
...
class Entry(Base):
    ...
    def __repr__(self):
        '''An alternative to __repr__ for a more formalized display'''
        #self.entry_name = self.entry_name.center()
        return """Entry
{:<20}{:}
{:<20}{:>20}
{:<20}{:>20}
...
{:<20}{:>20}
{:<20}{:>20}
{:<20}{:>10}""".format('Entry ID:', self.entry_id,
                            'Entry name:', self.entry_name, 'Date:', self.get_date_formal, 'Link:', 
                            self.entry_url, 'Category:', str(self.category), 'Publication:',
                            str(self.publication), 'Authors:', str(self.author_names2),
                            'Keywords:', str(self.keyword_list2),'Description:', self.description)
    #we use the format string here for greater control over the display
    
    @property
    def author_names2(self):
        '''List of the authors with just the names. Needed for __repr__'''
        return [i.name for i in self.authors]
    
    @property
    def keyword_list2(self):
        '''A list of just the keywords, no keyword IDs. Needed for __repr__'''
    ...
    @property
    def get_date_formal(self):
        date_f = self.date.strftime("%B %d, %Y (%A)")
        return date_f
